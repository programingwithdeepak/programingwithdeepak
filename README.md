# Hi 👋, I'm Deepak Chaudhary

### 💻 Full Stack Developer | MERN Stack Enthusiast

Passionate about building scalable web applications and solving real-world problems using modern technologies. I enjoy working on **frontend, backend, and AI-based projects**.

---

## 🚀 About Me
-🎓 Pursuing B.Tech in Computer Science from IMS Engineering College, Ghaziabad
- 🔭 I’m currently working on **Web Development Projects (MERN Stack)**
- 🛡️ Built a project: **AI-Powered Cybersecurity Threat Detection Dashboard**
- 🌱 I’m currently learning **Advanced MERN Stack & System Design**
- 👯 I’m looking to collaborate on **Open Source & Web Projects**
- 💬 Ask me about **React, JavaScript, Redux, Node.js, Express, MongoDB**
- 📫 How to reach me: **deepakchaudhary7830230168@gmail.com**
- ⚡ Fun fact: I love turning **ideas into real web applications**

---

## 🛠️ Tech Stack

### 🎨 Frontend
- HTML  
- CSS  
- JavaScript  
- React  
- Redux  
- Tailwind CSS  
- Bootstrap  

### ⚙️ Backend
- Node.js  
- Express.js  

### 🗄️ Database
- MongoDB  
- MySQL  

### 💻 Programming Languages
- JavaScript  
- Java  

### 🧰 Tools
- Git  
- GitHub  
- VS Code  
- Postman  

---

## 📂 Projects

### 🛡️ AI-Powered Cybersecurity Threat Detection Dashboard
- Detects and monitors cybersecurity threats using AI concepts.
- Dashboard interface to visualize threat data.
- Built using modern web technologies.

---
## 💻 Top Languages

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=programingwithdeepak&layout=compact&theme=tokyonight)
---

## 🌐 Connect With Me

- 💼 LinkedIn: https://www.linkedin.com/in/programingwithdeepak/  
- 📧 Email: deepakchaudhary7830230168@gmail.com  
- 🌍 Portfolio: Add your portfolio link  

---


⭐ *Always learning new technologies and building impactful projects.*
